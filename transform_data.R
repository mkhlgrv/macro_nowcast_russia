stat_data <- 1:3 %>% map_dfr(function(i){
  
  allraw <- import(file = 'data/raw data/all raw.xlsx', sheet = i)
  agg_info <- import('data/transformed/desc.xlsx')[c('name', 'aggregation', 'stationarity')]
  pub_lags <- import('data/transformed/desc.xlsx')[c('name', 'lag')]
  d1 <- allraw %>% 
    mutate(date = as.Date(date) %>% get.next.weekday(6),
           quarter_first_friday = as.yearqtr(date) %>% as.Date %>% get.next.weekday(6),
           h = as.numeric(date - quarter_first_friday)/7) %>%
    melt(id.vars=c('date','quarter_first_friday','h' )) %>%
  ...
  quarter_end_data <- d2 %>% group_by(quarter_first_friday, variable) %>%
    filter(h == max(h)) %>%
    mutate(next_year_quarter_first_friday = as.yearqtr(quarter_first_friday +390) %>%
             as.Date%>%
             get.next.weekday(6)) %>%
    ungroup %>%
...
save(stat_data, file="data/stat_data.Rdata")
